      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            ResponsiveSize.isMobile(context) ? Text("We're on mobile") : Text("We're not on mobile"),

            /// Here we use the function getResponsiveSize to change the size of the text depending on the device
            Text("This text has a responsive font size !", style: TextStyle(fontSize: ResponsiveSize.getResponsiveSize(context, 15))),

            /// Here is the same as above, but the scale factor will be lower
            Text("This text has a smaller responsive font size !", style: TextStyle(fontSize: ResponsiveSize.getSmallerResponsiveSize(context, 15))),
            Container(
              height: ResponsiveSize.getResponsiveSize(context, 500),
              child: GridView.count(
                crossAxisSpacing: ResponsiveSize.getResponsiveSize(context, 10),
                mainAxisSpacing: ResponsiveSize.getSmallerResponsiveSize(context, 10),
                ...
                crossAxisCount: ResponsiveSize.responsiveGridColumnCount(context),
                children: [
                  Container(color: Colors.grey, child: Center(child: Text("This grid view has a responsive crossAxisCount !"),),),
                  Container(color: Colors.grey, child: Center(child: Text("This grid view has a responsive crossAxisCount !"),),),
                  Container(color: Colors.grey, child: Center(child: Text("This grid view has a responsive crossAxisCount !"),),),
                  Container(color: Colors.grey, child: Center(child: Text("This grid view has a responsive crossAxisCount !"),),),
                  Container(color: Colors.grey, child: Center(child: Text("This grid view has a responsive crossAxisCount !"),),),
